...
export const transactions = sqliteTable('transactions', {
  ...
  /**
   * Метка строки банковской выписки: `PFM:{сумма}:{дата}:{номер}`.
   *
   * Отличает импортированное от заведённого руками. На совпадение с новой
   * строкой выписки претендует только ручная операция: иначе две покупки на
   * одну сумму в одном окне склеились бы в одну.
   */
  importId: text('import_id'),

  /**
   * Часть сплита: одна покупка, разложенная по нескольким категориям.
   *
   * Со счёта уходит родитель, по категориям расходятся части. Стороны не
   * пересекаются — баланс смотрит только на родителя, бюджет только на части,
   * — поэтому одна и та же сумма не считается дважды. У родителя категории
   * нет: её заменяют части, и считать его нераспределённой тратой нельзя.
   */
