...
Одна покупка на несколько категорий — родитель и части. Со счёта уходит
**родитель**, по категориям расходятся **части**; стороны не пересекаются,
поэтому одна сумма не считается дважды. Всё, что считает баланс, обязано
фильтровать `parent_transaction_id IS NULL`; всё, что считает бюджет, и так
отсеивает родителя условием `category_id IS NOT NULL`.

Родитель — не «трата мимо категорий»: категории у него в частях, и в сверке он
исключён из `uncategorized`. В ленте части не показываются отдельными строками:
в банке это одна покупка. Удаление родителя уносит части.
...
`@pfm/mcp` owns a declarative table of 64 tools; each maps arguments to an HTTP
